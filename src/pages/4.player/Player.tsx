...
import { Route } from "@/routes/player/$id";
import { useQuery } from "@tanstack/react-query";

export function Player() {
  const { id } = Route.useParams();
  const {
    isPending: isPlayerInfoPending,
    isError: isPlayerInfoError,
    error: playerInfoError,
    data: playerInfoData,
  } = useQuery({
    queryKey: ["player_info", id],
    queryFn: async () => {
      const response = await fetch(`http://localhost:8000/player_info/${id}`);
      if (!response.ok) {
